...
namespace UnityEngine.XR.Content.Interaction
{
    ...
    public class XRLever : XRBaseInteractable
    {
        ...
        void UpdateValue()
        {
            ...
            m_LookAngle = Mathf.Atan2(lookDirection.z, lookDirection.y) * Mathf.Rad2Deg;

            if (m_MinAngle < m_MaxAngle)
                m_LookAngle = Mathf.Clamp(m_LookAngle, m_MinAngle, m_MaxAngle);
            else
                m_LookAngle = Mathf.Clamp(m_LookAngle, m_MaxAngle, m_MinAngle);

            var maxAngleDistance = Mathf.Abs(m_MaxAngle - m_LookAngle);
            var minAngleDistance = Mathf.Abs(m_MinAngle - m_LookAngle);
